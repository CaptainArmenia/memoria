...
def validate_labels(labels, type_id) :
    ...
    if (type_id == 1 or len(label_set) == max(label_set) + 1) and (min(label_set) == 0):
        return new_labels
    else:            
        raise ValueError("Some codes are missed in label set! {}".format(label_set))
    
def read_data_from_file(str_path, type_id, dataset = "train", shuf = True):
    ...
    with open(datafile) as file :        
        ...
        #print(type_id)
        labels = validate_labels(labels, type_id)
...
def create_tfrecords(data_dir, type_id, image_shape, processFun = imgproc.process_image):    
    ...
    if ( type_id + 1 ) & 1 : # train   ( 0 + 1 ) & 1  == 1 
        filenames, labels = read_data_from_file(data_dir, type_id, dataset="train", shuf = True)
    ...
    if ( type_id + 1 ) & 2 : # test ( 1 + 1 ) & 2  == 2
        filenames, labels = read_data_from_file(data_dir, type_id, dataset="test", shuf = True)
